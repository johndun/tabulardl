...
class Dataset(BaseExperimentDataset):
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            sample_ids = self.user_sample_id_map[idx][:-1]
        else:
            sample_ids = self.user_sample_id_map[idx]

        all_samples = set(self.user_sample_id_map[idx])
        interactions = self.interaction[sample_ids]
        item_ids = [self.interaction_item_map[x] for x in sample_ids]
        items = self.item[item_ids]
        src_interactions = {key: val[:-1] for key, val in interactions.items()}
        src_items = {key: val[:-1] for key, val in items.items()}
        tgt_items = {key: val[1:] for key, val in items.items()}
        tgt_len = len(item_ids) - 1
        negative_sample_ids = np.random.choice(
            list(self.item_ids - all_samples),
            size=tgt_len if self.split == 'train' else self.size,
            replace=False
        )
        neg_items = self.item[negative_sample_ids]
        mask = torch.zeros(self.seq_len)
        if tgt_len > self.seq_len:
            src_interactions = {key: val[-self.seq_len:] for key, val in src_interactions.items()}
            src_items = {key: val[-self.seq_len:] for key, val in src_items.items()}
            if self.split == 'train':
                tgt_items = {key: val[-self.seq_len:] for key, val in tgt_items.items()}
                neg_items = {key: val[-self.seq_len:] for key, val in neg_items.items()}
        else:
            n_to_pad = self.seq_len - tgt_len
            feature_sets = (
                [src_interactions, src_items, tgt_items, neg_items]
                if self.split == 'train' else
                [src_interactions, src_items]
            )
            for feature_set in feature_sets:
                for key, val in feature_set.items():
                    shp = list(val.shape)
                    shp[0] = n_to_pad
                    feature_set[key] = torch.concat((torch.zeros(shp, dtype=val.dtype), val), dim=0)
            mask[:n_to_pad] = 1
        if self.split != 'train':
            tgt_items = {key: val[-1:] for key, val in tgt_items.items()}
...
class DataProvider(BaseDataProvider):
    ...
    def initialize(self, features: DataSchema):
        artifact_paths = [
